  ...
  public class PbfFast
  {
    ...
    static int DecodeDenseNodes(byte[] buf, int ofs, string[] stringTable, OsmNode[] nodes, ref int nodesOfs)
    {
      ...
      for (int i = 0, nodeIndex = 0; i < nodeKeys.Length; i++, nodeIndex++)
      {
        ...
        if (li > 0)
        {
          ...
          nodes[nodesOfs + nodeIndex] = new OsmNode(id[nodeIndex], (int)lat[nodeIndex], (int)lon[nodeIndex], t);
        }
        else
        {
    ...
    public static int DecodePrimitiveBlock(byte[] buf, int ofs, OsmBlob blob, out OsmNode[] gpsNodes)
    {
      ...
      while (buf[ofs + len] == (2 << 3 | 2))
      {
        ...
        while (len < endLen)
        {
          len += DecodePrimitiveGroup(buf, ofs + len, stringTable, gpsNodes, ref outputOfs);
        }
        if (len != endLen) throw new PbfParseException();
      }
    ...
    public static int DecodePrimitiveBlock(byte[] buf, int ofs, OsmBlob blob, out MemArray<OsmNode> gpsNodes)
    {
      /*****
       * message PrimitiveBlock
       * {
      ...
      // --- repeated PrimitiveGroup primitivegroup = 2; ---
      gpsNodes = new MemArray<OsmNode>((int)blob.nodeCount);
      int outputOfs = 0;
      while (buf[ofs + len] == (2 << 3 | 2))
      {
        len++;
        ...
        while (len < endLen)
        {
          len += DecodePrimitiveGroup(buf, ofs + len, stringTable, gpsNodes.RawData, ref outputOfs);
        }
        if (len != endLen) throw new PbfParseException();
      }
      if (outputOfs != gpsNodes.Length) throw new IndexOutOfRangeException();
